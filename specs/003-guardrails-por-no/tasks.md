...
- **Feedback pós-implementação do usuário**: os testes de integração desta feature (e um pré-existente reaproveitado, `test_prompt_manager_sync.py`) inseriam prompts/guardrails/vínculos reais no Postgres de dev sem limpar depois, poluindo a tabela entre execuções. Corrigido adicionando `tests/integration/conftest.py` com um fixture `db_cleanup` (teardown via `yield`, roda mesmo se o teste falhar no assert) que rastreia tudo que cada teste cria (`track_prompt`/`track_guardrail`/`track_tenant`) e apaga em ordem segura de FK (`tenant_prompts`/`prompt_guardrails` → `prompts`/`guardrails`) ao final. Aplicado em todos os arquivos de integração que tocam o banco real: `test_prompt_manager_sync.py`, `test_prompt_manager_node_type.py`, `test_prompt_manager_node_type_api.py`, `test_chitchat_node_guardrails.py`, `test_institutional_node_guardrails.py`, `test_prompt_manager_seed.py`. Isso expôs um assert antigo (`total >= 3`) em `test_returns_only_active_when_multiple_exist` que só passava por depender de sobra de execuções anteriores — corrigido para vincular explicitamente A/B/C dentro do próprio teste e checar `total == 3`. Validado rodando a suíte duas vezes seguidas: mesmo resultado nas duas, zero linha `%EDI%` restante no banco ao final.
