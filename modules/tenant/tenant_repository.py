...
class TenantRepository:
    ...
    def create_tenant(self, tenant_data) -> dict:
        ...
        create_query = """
        INSERT INTO tenants (id, name, google_calendar_id, created_at)
        VALUES (%s, %s, %s, NOW())
        RETURNING id, name, google_calendar_id, created_at;
        """
        cursor = self.db_connection.cursor()
        cursor.execute(create_query, (tenant_data['tenant_id'], tenant_data['name'], tenant_data['google_calendar_id']))
        ...
        return {
            ...
            'created_at': new_tenant[3]
        }

    def get_tenant(self, tenant_id) -> dict:
        # Logic to retrieve a tenant by ID from the database
        get_query = "SELECT id, name, google_calendar_id, created_at FROM tenants WHERE id = %s;"
        cursor = self.db_connection.cursor()
    ...
    def update_tenant(self, tenant_id, tenant_data) -> dict | None:
        ...
        update_query = """
        UPDATE tenants  
        SET name = %s, google_calendar_id = %s, updated_at = NOW()
        WHERE id = %s
        RETURNING id, name, google_calendar_id, updated_at;
        """
        cursor = self.db_connection.cursor()
        cursor.execute(update_query, (tenant_data['name'], tenant_data['google_calendar_id'], tenant_id))
